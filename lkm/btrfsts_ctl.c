...
static void usage(const char *prog)
{
	fprintf(stderr,
		...
		"\n"
		"Options:\n"
		"  --device  PATH   block device or mount point of btrfs fs\n"
		"  --inode   NUM    target inode number\n"
		"  --path    FILE   target file path (preferred for btrfs)\n"
		...
		"  --help\n"
		"\n"
		"Requires the btrfs_timestomp kernel module to be loaded.\n",
		prog);
}

static void print_ts(const char *label, const struct btrfsts_ts *ts, const char *src)
{
	if (!ts->valid) {
		printf("  %-8s  (unchanged)\n", label);
		return;
...
int main(int argc, char *argv[])
{
	...
	unsigned int ioctl_cmd = BTRFSTS_STOMP;
	...
	fd = open(BTRFSTS_DEV_PATH, O_RDWR);
	if (fd < 0) {
		perror("open " BTRFSTS_DEV_PATH);
		fprintf(stderr, "Is the btrfs_timestomp module loaded? "
			"(sudo insmod btrfs_timestomp.ko)\n");
		return 1;
	}

	if (ioctl(fd, ioctl_cmd, &req) < 0) {
		perror(ioctl_cmd == BTRFSTS_STOMP_PATH ? "ioctl BTRFSTS_STOMP_PATH" : "ioctl BTRFSTS_STOMP");
		close(fd);
		return 1;
	}
	close(fd);
